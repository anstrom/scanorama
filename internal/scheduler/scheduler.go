...
func (s *Scheduler) AddDiscoveryJob(ctx context.Context, name, cronExpr string, config DiscoveryJobConfig) error {
	...
	log.Printf("Added discovery job '%s' with schedule '%s'", name, cronExpr)
	return nil
}
...
func (s *Scheduler) AddScanJob(ctx context.Context, name, cronExpr string, config ScanJobConfig) error {
	...
	log.Printf("Added scan job '%s' with schedule '%s'", name, cronExpr)
	return nil
}
...
func (s *Scheduler) loadScheduledJobs() error {
	...
	for rows.Next() {
		...
		// Add to cron scheduler based on type
		var cronID cron.EntryID
		if job.Type == db.ScheduledJobTypeDiscovery {
			var config DiscoveryJobConfig
			if err := json.Unmarshal([]byte(job.Config), &config); err != nil {
				log.Printf("Failed to unmarshal discovery config for job %s: %v", job.Name, err)
				continue
			}

			cronID, err = s.cron.AddFunc(job.CronExpression, func() {
				s.executeDiscoveryJob(job.ID, config)
			})
		} else if job.Type == db.ScheduledJobTypeScan {
			var config ScanJobConfig
			if err := json.Unmarshal([]byte(job.Config), &config); err != nil {
				log.Printf("Failed to unmarshal scan config for job %s: %v", job.Name, err)
				continue
			}
		...
		if err != nil {
			log.Printf("Failed to add cron job for %s: %v", job.Name, err)
			continue
		}

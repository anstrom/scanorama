...
ALTER TABLE scan_jobs ADD CONSTRAINT IF NOT EXISTS check_job_timing
    CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at);

ALTER TABLE hosts ADD CONSTRAINT IF NOT EXISTS check_confidence_range
    CHECK (os_confidence IS NULL OR (os_confidence >= 0 AND os_confidence <= 100));

ALTER TABLE scan_jobs ADD CONSTRAINT IF NOT EXISTS check_progress_range
    CHECK (progress_percent >= 0 AND progress_percent <= 100);

-- Performance indexes for common query patterns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_port_scans_job_host
    ON port_scans (job_id, host_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_port_scans_host_port_state
    ON port_scans (host_id, port, state) WHERE state = 'open';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scan_jobs_status_created
    ON scan_jobs (status, created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scan_jobs_target_status
    ON scan_jobs (target_id, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hosts_status_last_seen
    ON hosts (status, last_seen) WHERE status = 'up';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hosts_os_family_status
    ON hosts (os_family, status) WHERE os_family IS NOT NULL AND status = 'up';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_port_scans_scanned_at
    ON port_scans (scanned_at);

-- Composite index for dashboard queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hosts_discovery_status
    ON hosts (discovery_method, status, last_seen)
    WHERE discovery_method IS NOT NULL;

-- Index for scheduled job processing
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scheduled_jobs_next_run_enabled
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_host_history_host_created
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scheduled_jobs_failures
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_port_scans_recent
    ON port_scans (host_id, port, state)
    WHERE scanned_at > NOW() - INTERVAL '30 days' AND state = 'open';

-- Add index for service detection queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_port_scans_service_detection
    ON port_scans (service_name, service_version)
    WHERE service_name IS NOT NULL;

-- Add constraint to ensure scan jobs have proper status progression
ALTER TABLE scan_jobs ADD CONSTRAINT IF NOT EXISTS check_status_timing
    CHECK (
...
-- Add index to support the performance view
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scan_jobs_created_date
    ON scan_jobs (DATE_TRUNC('day', created_at), status);

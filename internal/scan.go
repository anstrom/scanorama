...
func createAdhocScanTargetTx(ctx context.Context, tx *sqlx.Tx, target string, config *ScanConfig) (*db.ScanTarget, error) {
	// Parse target address
	networkAddr, err := parseTargetAddress(target)
	if err != nil {
		return nil, err
	...
	err = tx.QueryRowContext(ctx, query, scanTarget.ID, scanTarget.Name, scanTarget.Network,
		...
		scanTarget.ScanType, scanTarget.Enabled).Scan(&scanTarget.CreatedAt, &scanTarget.UpdatedAt)
	if err != nil {
		return nil, fmt.Errorf("failed to create scan target: %w", err)
	}

...
.PHONY: help build clean clean-test test test-up test-down test-logs coverage lint lint-install lint-fix deps install run fmt vet check all
...
coverage: test-up ## Generate test coverage report
	...
	@$(GOTEST) -cover ./... -coverprofile=$(COVERAGE_FILE) ; ret=$$? ; \
	if [ $$ret -eq 0 ]; then \
